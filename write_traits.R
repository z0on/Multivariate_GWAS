...
write.table(pd,file="pd.traits",quote=F,row.names=F,sep="\t")

bleach=bigtraits[,c(2,6,4,5)]
head(bleach)
write.table(bleach,file="bleach.traits",quote=F,row.names=F,sep="\t")
...
head(mds)
write.table(mds,file="mds7",quote=F,row.names=F,sep="\t")